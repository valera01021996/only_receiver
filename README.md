...
Сервис разбирает имя файла по `_` и извлекает:
- `<phone>` — 4-й сегмент (`parts[3]`), номер отправителя для проверки по белому списку
- `<HHMMSS>` — 2-й сегмент (`parts[1]`); используется как хвостовой компонент `post_id` (см. ниже)

> **Примечание:** несмотря на название переменной `file_idx` в коде, фактически в неё попадает `parts[1]` — временная метка `<HHMMSS>` из имени файла, а не сегменты `<seq>` или хвостовой `<idx>`. Похоже на неточность в [`parse_from_filename`](tasks.py), а не задуманное поведение.
...
Где:
- `<HH:MM:SS>` — **текущее время обработки** файла (момент вызова `build_post_id`), а не время из имени файла
- `<tz_offset>` — всегда `+05:00`, жёстко задан в коде
- `<file_idx>` — 2-й сегмент имени файла, т.е. фактически `<HHMMSS>` из имени файла (см. примечание выше)

Пример (файл `IN20240315_143022_00_+998909192558_00.txt`, обработан в 18:42:10):
```
sms:+998909192558:18:42:10+05:00:143022
```
